...
## Zgodność z programem JSystems (Day 4)

- **Moduł 4.1: AI-Generated Testing**
  - Pokrycie w skrypcie: `09:20–10:05` (projektowanie testów z kryteriów), `10:05–11:00` (review diffu testów i zmian), `14:40–15:25` (iteracje naprawcze po findings).
  - Efekt zgodny z programem: uczestnik wychodzi z praktycznym zestawem testów i umie je poprawiać z AI zamiast bezrefleksyjnie akceptować.
- **Moduł 4.2: Automated Code Reviews & Security**
  - Pokrycie w skrypcie: `10:05–11:00` (review wygenerowanego kodu), `11:15–12:05` (security checkpoint + legacy JFTP audit), `14:40–15:25` (domknięcie findings).
  - Efekt zgodny z programem: PR/zmiana jest oceniana przez quality + security gate, z jawną klasyfikacją ryzyk i decyzją co naprawiamy teraz.
- **Moduł 4.3: AI for CI/CD and Automation**
  - Pokrycie w skrypcie: `13:30–14:20` (cloud-max vs on-prem workflow), wsparcie w `14:20–14:30` (narzędzia jako most, nie chaos).
  - Efekt zgodny z programem: uczestnik projektuje realistyczny pipeline z testami, review, security gate i approval, gotowy do adaptacji enterprise.
...
## Prompty trenerskie Day 4 (inline, gotowe do wklejenia)

### P1 — Testy z kryteriów akceptacji (09:20)
```text
Pracujemy na poniższym fragmencie funkcji/endpointu. 
Nie zmieniaj jeszcze kodu produkcyjnego.

Zadanie:
1) Wyciągnij 5 kryteriów akceptacji (3 pozytywne, 2 negatywne).
2) Zaproponuj zestaw testów unit/integration mapujący 1:1 do kryteriów.
3) Oznacz testy: MUST (krytyczne) vs NICE (opcjonalne).
4) Wypisz ryzyko regresji, którego te testy NIE pokrywają.

Format odpowiedzi:
- tabela: Kryterium | Typ testu | Priorytet | Uzasadnienie
- na końcu: "czego jeszcze brakuje"
```

### P2 — Generowanie testów bez scope creep (09:35)
```text
Na podstawie ustalonych kryteriów wygeneruj testy dla [nazwa modułu].
Zasady twarde:
- NIE zmieniaj kodu produkcyjnego,
- NIE dodawaj nowych bibliotek,
- NIE refaktoruj innych plików.

Po wygenerowaniu:
- wypisz listę plików, które zmieniłeś,
- krótko uzasadnij każdy test,
- wskaż, które testy mogą być flaky i dlaczego.
```

### P3 — Review diffu pod ryzyko (10:05)
```text
Przeanalizuj poniższy diff jak reviewer senior:
1) Czy zakres zgadza się z zadaniem?
2) Jakie są 3 główne ryzyka?
3) Co powinno być cofnięte lub zawężone?
4) Jak sformułować feedback do autora/agenta w max 6 punktach?

Chcę odpowiedź praktyczną, bez ogólników.
Format: KEEP / FIX NOW / MOVE TO BACKLOG.
```

### P4 — Prompt do iteracji naprawczej po review (10:35)
```text
Wykonaj poprawkę tylko w zakresie poniżej:
- Problem: [wklej finding]
- Zakres do zmiany: [konkretne pliki/fragmenty]
- Zakres zakazany: [czego nie ruszać]
- Kryterium akceptacji poprawki: [test/check]

Po zmianie:
1) pokaż minimalny diff,
2) napisz 3-zdaniowe uzasadnienie,
3) potwierdź, że zakres zakazany został nienaruszony.
```

### P5 — Security audit (legacy JFTP checkpoint) (11:15)
```text
Zrób security audit poniższego fragmentu legacy (JFTP).
Szukaj tylko ryzyk realnych dla środowiska enterprise/bankowego:
- walidacja danych wejściowych,
- auth/authz,
- sekrety,
- logowanie danych wrażliwych,
- zależności,
- potencjalny prompt injection / command injection.

Dla każdego ryzyka podaj:
- severity: CRITICAL/HIGH/MEDIUM/LOW,
- decyzja: FIX NOW / NEXT SPRINT,
- minimalną poprawkę.
```

### P6 — Plan mikro-refaktoru legacy (12:05)
```text
Przygotuj plan bezpiecznej modernizacji legacy modułu [X] w 3 iteracjach.
Warunki:
- każda iteracja <= 1 dzień pracy,
- po każdej iteracji musi być check regresji,
- bez "big-bang rewrite".

Format:
Iteracja | Cel | Zakres | Ryzyko | Test/Check | Kryterium stop.
```

### P7 — CI/CD cloud vs on-prem (13:30)
```text
Zaproponuj dwa warianty pipeline dla tego projektu:
A) cloud-max (GitHub Actions + agent CLI),
B) on-prem (Jenkins/GitHub Enterprise + lokalny model lub custom endpoint).

Dla obu wariantów podaj:
- etapy pipeline,
- gdzie jest security gate,
- gdzie jest manual approval,
- jakie artefakty audytowe zapisujemy.

Nie rób deep infra dive — ma być wersja szkoleniowa, realistyczna.
```

### P8 — Finalny prompt naprawczy na koniec dnia (14:40)
```text
Na bazie findings z testów/review/security przygotuj jedną minimalną iterację naprawczą.
Priorytet: największe ryzyko przy najmniejszym zakresie zmiany.

Wynik ma zawierać:
1) krótki plan (max 5 punktów),
2) proponowany commit message,
3) checklistę "done" (test, review, security check).

Jeśli ryzyko jest zbyt duże na dziś — oznacz NO-GO i podaj dlaczego.
```

## Szybkie copy-paste dla trenera (Day 4)

### 1) Otwarcie dnia (09:00)
```text
Day 4: przechodzimy z "działa" do "działa i można to obronić".
Dziś pracujemy pętlą: kryteria → testy → review diffu → security check → poprawka.
Cel: mniej chaosu, więcej przewidywalności i gotowość na realny audyt.
```

### 2) Instrukcja do testów (09:20)
```text
Najpierw kryteria akceptacji, potem testy.
Nie piszemy testów „bo ładnie wyglądają”, tylko takich, które chronią przed realną regresją.
Skupiamy się na błędach, walidacji i edge-case’ach.
```

### 3) Instrukcja do review (10:05)
```text
Patrzymy na diff warstwami:
1) zakres,
2) logika,
3) side effects,
4) ryzyka.
Feedback do agenta ma być krótki, precyzyjny i ograniczający scope.
```

### 4) Security checkpoint (11:15)
```text
Obowiązkowy checkpoint: legacy JFTP audit.
Każde ryzyko klasyfikujemy: CRITICAL/HIGH/MEDIUM/LOW.
Dla CRITICAL/HIGH decyzja musi być jawna: FIX NOW albo NEXT SPRINT.
```

### 5) Legacy bez heroizmu (12:05)
```text
Nie robimy rewolucji. Robimy mikro-kroki:
mapa modułu → mała zmiana → test/regresja → kolejna iteracja.
AI pomaga, ale tempo i zakres kontroluje człowiek.
```

### 6) Cloud vs on-prem (13:30)
```text
Cloud i on-prem różnią się narzędziami, ale nie logiką procesu.
W obu światach zostaje ten sam rdzeń:
prompt → diff → test → review → approval → deploy.
```

### 7) Zamknięcie dnia (15:25)
```text
Po Day 4 mamy nie tylko kod, ale proces jakościowy.
Jutro (Day 5): finalne demo, decyzje techniczne i plan wdrożenia po szkoleniu.
Zabieracie z tego dnia praktyki, które można wdrożyć od razu w zespole.
```
